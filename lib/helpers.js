...
const encryptObject = async (model) => {
    ...
    Object.keys(model.schema).forEach((key) => {
        ...
        let ivdata = '';
        let kData = '';
        crypto_1.default.randomBytes(16, function (err, buffer) {
            ivdata = buffer.toString('hex');
        });
        crypto_1.default.randomBytes(32, function (err, buffer) {
        ...
        const cipher = crypto_1.default.createCipheriv('aes-256-cbc', keyData.key, keyData.iv);
