  ...
  <div class='pad1'>
    ...
    <div class='clearfix'>
      <div class='fl pad1y space-right2'>
        ...
        <span class='fraction'>138/172</span>
      </div>
      <div class='fl pad1y space-right2'>
...
<a name='L722'></a><a href='#L722'>722</a></td><td class="line-coverage quiet"><span class="cline-any cline-neutral">&nbsp;</span>
...
export function encryptContent(
  ...
  if (!opts.publicKey) {
    const privateKey = (caller || <span class="branch-1 cbranch-no" title="branch not covered" >new UserSession())</span>.loadUserData().appPrivateKey
...
function normalizeOptions&lt;T&gt;(
  ...
  if (opts.username) {
    if (!opts.app) {
      const appConfig = (caller || <span class="branch-1 cbranch-no" title="branch not covered" >new UserSession())</span>.appConfig
      if (!appConfig) {
...
<span class="cstat-no" title="statement not covered" >          throw new Error(`getFile ${path} failed with HTTP status ${response.status}`)</span>
        }
      }
      const contentType = response.headers.get('Content-Type')
      if (forceText || contentType === null
          || contentType.startsWith('text')
          || <span class="branch-3 cbranch-no" title="branch not covered" >contentType === 'application/json')</span> {
...
&nbsp;
    try {
      ...
      const fileUrls = <span class="cstat-no" title="statement not covered" >await Promise.all([</span>
        uploadToGaiaHub(path, content, freshHubConfig, contentType),
        uploadToGaiaHub(`${path}${SIGNATURE_FILE_SUFFIX}`,
                        signatureContent, freshHubConfig, 'application/json')
      ])
<span class="cstat-no" title="statement not covered" >      return fileUrls[0]</span>
    }
  }
&nbsp;
